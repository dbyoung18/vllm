...
class SqueezeLLMLinearMethod(LinearMethodBase):
    ...
    def apply_weights(self,
        ...
        if is_hip():
            out_f = torch.zeros(out_shape, device=get_accelerator().device_name(), dtype=torch.float)
            ops.squeezellm_gemm(reshaped_x, qweight, out_f, lookup_table)
            out = out_f.to(dtype=torch.float16)
        else:
            # NOTE: The output tensor should be zero-initialized.
            out = torch.zeros(out_shape, device=get_accelerator().device_name(), dtype=torch.float16)
