...
def parse_args():
    ...
    parser.add_argument("--input_len", type=int, default=1, help="Input prompt length.")
...
def profile_handler(
    ...
    logger.info(f"Exporting {profile_prefix} to {profile_dir}")
    save_profile(
...
if __name__ == "__main__":
    ...
    if args.num_beams == 1:
        ...
        sampling_params = SamplingParams(
            use_beam_search=True, best_of=args.num_beams, top_k=-1, temperature=0
        )  # BeamSearch
    ...
    if args.device_type == "xpu" and args.optimize_transformers:
        ...
        for idx, worker in enumerate(llm.llm_engine.workers):
            llm.llm_engine.workers[idx].model = ipex.optimize_transformers(
                worker.model, dtype=args.data_type, inplace=True, device=args.device_type
            )
